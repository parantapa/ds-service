...
Two other generated artifacts are *not* covered by the C++ build:

- `cpp/task_table.hpp` is generated from `misc/task_table.json5` by the
    external `cppsoa` tool; regenerate it with `scripts/gen_cppsoa.sh`.
- The Python client stubs are generated by `scripts/gen_python_bindings.sh`.

Both are committed to the repository, so they only need regenerating when
