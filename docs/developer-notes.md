...
as opposed to people using it.
Everything here is about the mechanics of changing the code:
what is generated, what has to be regenerated by hand,
and which checks a change is expected to pass.

## The user-facing docs are the source of truth

The documents indexed in the README describe how the service behaves,
and that behaviour is deliberately not repeated here.
Read the one covering an area before changing code in it.

When behaviour changes,
update the document that covers it
rather than describing the new behaviour here.
...
- C++ formatting is enforced by `.clang-format` in the repository root:
    LLVM base, four-space indent, 120 columns, left pointer alignment.
    Note `SortIncludes: false` -- the include order is intentional,
    so do not let an editor reorder it.
...
- Use semantic line breaks in documentation, block comments,
    and docstrings.
- Markdown in this repository is plain ASCII.
...
including after it reaches `Complete`,
...
State is not persisted in any case,
so a restart is the current remedy.
